...
사용자에게는 한 덩어리 raw text 질문 대신 아래 순서로 묻는다. 목적 먼저 묻고 답변을 받은 뒤 자료 위치를 묻는다. 두 질문을 한 번에 묻지 않는다.

1. 프로젝트 목적은 자유서술로 요청한다.
   - 예: "이 프로젝트의 목적을 3~10문장 정도로 설명해주세요. 왜 시작했는지, 최종 산출물이 무엇인지, 누가 사용할 것인지, 성공 기준이 무엇인지가 포함되면 좋습니다."
2. 관련 자료 위치는 가능한 경우 Claude/Codex의 native option/choice UI로 묻는다.
   - Codex에서는 사용 가능한 경우 `request_user_input` 같은 선택형 질문 도구를 사용한다.
   - Claude/Codex 기타 환경에서는 해당 앱이 제공하는 선택형 질문 기능을 우선 사용한다.
...
- Claude 앱/웹에서는 공식 공개 기능으로 Agent가 Claude 앱 + 버튼을 직접 트리거한다고 가정하지 않는다. 사용자가 채팅창 왼쪽 아래 `+` 버튼 또는 프로젝트 Files 섹션에서 직접 자료를 추가하도록 안내한다.
- Claude Code에서는 로컬 폴더 참조를 위해 `/add-dir <path>`, 실행 시 `--add-dir <path>`, 또는 설정의 `additionalDirectories`를 안내한다.
- macOS 로컬 GUI와 shell 실행이 실제로 가능하면 아래 방식으로 `osascript` 선택적 시도를 통해 폴더 선택 창을 열 수 있다.

```sh
osascript -e 'POSIX path of (choose folder with prompt "OneDrive/SharePoint reference folder를 선택하세요")'
```

- 사용자가 취소하거나 GUI, 권한, 실행 환경 문제로 실패하면 OneDrive/SharePoint 폴더 경로를 직접 붙여넣게 한다.
- 받은 경로가 로컬에서 접근 가능하면 존재 여부를 확인하고, 초기화 결과와 `system/INDEX.md`에 reference source로 기록한다.
- 폴더 선택 창은 편의 기능일 뿐 필수 의존성이 아니며, 모든 환경에서 경로 입력 fallback으로 초기화가 가능해야 한다.
